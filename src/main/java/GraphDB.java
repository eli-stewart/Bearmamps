...
public class GraphDB {
    ...
    private KDTree tree;
    //public PrevClosest closest = new PrevClosest();
    private HashMap<String, Long> close = new HashMap<>();
    ...
    public class Edge {
        ...
        public Edge(long n1, long n2, double dist, long w) {
            if (n1 == n2) {
                System.out.println("Must create an Edge with two different Nodes");
                return;
            }
    ...
    private void clean() {
        ...
        for (long nodeID : nodes.keySet()) {
            if (nodes.get(nodeID).getEdges().isEmpty()) {
                idToRemove.add(nodeID);
            }
        }

        for (long toRemove : idToRemove) {
            nodes.remove(toRemove);
        }
        long startTime = System.currentTimeMillis();
        tree = new KDTree(this);
        long endTime = System.currentTimeMillis();
        System.out.println("Build took " + (endTime - startTime) + " milliseconds");
    }
    ...
    double lon(long v) {
        if (!nodes.containsKey(v)) {
            System.out.println("No Node with that ID exists");
            return 0;
        }
    ...
    Iterable<Long> adjacent(long v) {
        return nodes.get(v).getEdges().keySet();
    }
    ...
    public long closest(double lon, double lat) {
        String toS = Double.toString(lon) + "," + Double.toString(lat);
        long ret;
        if (close.containsKey(toS)) {
            ret = close.get(toS);
        } else {
            FindNearest find = new FindNearest(lon, lat, tree.getRoot());
            find.findChampion(tree.getRoot(), tree.getRoot());
            ret = find.getChampion().getNodeID();
            close.put(toS, ret);
        }
        return ret;
    }
    ...
    public void connect(long nodeID1, long nodeID2, long wayID) {
        if (!nodes.containsKey(nodeID1) || !nodes.containsKey(nodeID2)) {
            System.out.println("Must create the Nodes before creating the Edge");
            return;
        }

        double dist = distance(nodeID1, nodeID2);

        Node node1 = nodes.get(nodeID1);
        Node node2 = nodes.get(nodeID2);
        Edge newEdge = new Edge(nodeID1, nodeID2, dist, wayID);
